...
export default function AuthModal({ isOpen, onClose }) {
  ...
  // Direct Google credential fields
  ...
  const handleGoogleSignIn = async () => {
    ...
    } else if (result?.error && result.error !== 'Popup closed by user') {
      setShowManualGoogle(true);
      setLocalError(result.error);
    }
  };
  ...
  const handleManualGoogleSubmit = async (e) => {
    ...
    if (!googleName.trim() && !googleEmail.trim()) {
      setLocalError('Please enter your Google display name or email address.');
      return;
    }
    ...
    const res = await loginWithGoogle({
      ...
      phoneNumber: googlePhone.trim() || null,
    });

    if (res?.success) {
      onClose();
    } else {
      setLocalError(res?.error || 'Failed to authenticate profile.');
    }
  };
  ...
  const handleSendOtp = async (e) => {
    ...
    if (res.success) {
      setOtpSent(true);
      setOtpMessage(res.message || 'Verification OTP code dispatched.');
      setTimeout(() => {
          ...
          <div className="auth-tab-pane">
            <div className="auth-provider-hero">
              ...
              <p className="auth-provider-description">
                Authenticate with your official Google account to access live multi-hazard telemetry, AI operational copilot, and geodetic response coordination.
              </p>
            </div>
            ...
            <div className="auth-google-action-container">
              ...
              <button
                ...
                onClick={() => setShowManualGoogle((prev) => !prev)}
              >
                {showManualGoogle ? 'Hide manual entry' : 'Or enter Google details directly'}
              </button>
            </div>

            {/* Direct Google details entry (fail-safe for unwhitelisted Vercel domains) */}
            {showManualGoogle && (
              <form onSubmit={handleManualGoogleSubmit} className="auth-form" style={{ marginTop: '0.5rem', borderTop: '1px solid var(--border-subtle)', paddingTop: '0.85rem' }}>
                <div className="auth-form-group">
                  <label htmlFor="auth-google-name" className="auth-field-label">
                    FULL NAME
                  </label>
                  <input
                    id="auth-google-name"
                    ...
                    placeholder="Enter your name (e.g. Aryan Das)"
                ...
                <div className="auth-form-group">
                  <label htmlFor="auth-google-email" className="auth-field-label">
                ...
                <div className="auth-form-group">
                  <label htmlFor="auth-google-phone" className="auth-field-label">
                    PHONE NUMBER (OPTIONAL)
                  </label>
                  <input
                    id="auth-google-phone"
                    type="tel"
                    value={googlePhone}
                    ...
                    className="auth-text-input"
                  />
                  <span className="auth-input-hint">Displayed in your profile card when clicked.</span>
                </div>
              ...
              <form onSubmit={handleVerifyOtp} className="auth-form">
                ...
                <div className="auth-form-group">
                  ...
                  <input
                    ...
                    className="auth-text-input"
                  />
                  <span className="auth-input-hint">Used for your personalized operator profile.</span>
                </div>
          ...
          <form onSubmit={handleAdminSubmit} className="auth-form auth-tab-pane">
            <div className="auth-form-group">
              ...
              <input
                ...
                placeholder="Enter Admin ID (default: abc123)"
                className="auth-text-input"
                required
              />
            </div>
            ...
            <div className="auth-form-group">
              ...
              <input
                ...
                placeholder="Enter Authorization Password (default: ERer00*#)"
                className="auth-text-input"
                required
              />
            </div>
