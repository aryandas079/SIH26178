...
function isGeographicallyAllowed(geography, hazardName, row = null) {
  if (!row) return geography.isLand

  const latitude = Number(row.latitude)
  const value = (input) => String(input || '').trim().toLowerCase()
  const hasThreat = (field) => {
    const level = value(row[field])
    return level !== '' && level !== 'none' && level !== 'good' && level !== 'excellent'
  }

  if (hazardName === 'Cyclone') {
    return geography.isLand && Math.abs(latitude) <= 30 && geography.coastlineDistanceKm <= cycloneCoastalLimitKm
  }

  if (hazardName === 'Tsunami') {
    return geography.isLand && geography.coastlineDistanceKm <= tsunamiCoastalLimitKm && hasThreat('tsunami_risk')
  }

  if (hazardName === 'Glacial Liquefaction') {
    return geography.isLand && Math.abs(latitude) >= 45 && hasThreat('glacial_liquefaction_risk')
  }

  if (hazardName === 'Water Quality') {
    return geography.isLand && ['moderate', 'poor', 'contaminated', 'high', 'severe', 'hazardous', 'very unhealthy', 'unhealthy'].includes(value(row.water_quality))
  }

  if (hazardName === 'Flood') {
    return geography.isLand && ['moderate', 'high', 'severe', 'hazardous'].includes(value(row.flood_risk))
  }

  if (hazardName === 'Extreme Heat') {
    return geography.isLand && Number(row.max_temp_c_forecast || 0) >= 32 && Math.abs(latitude) <= 70
  }

  if (hazardName === 'Forest Fires') {
    return geography.isLand && ['moderate', 'high', 'severe', 'hazardous'].includes(value(row.forest_fire_risk)) && Math.abs(latitude) <= 70
  }

  if (hazardName === 'Earthquakes') {
    return geography.isLand && Number(row.earthquake_magnitude_est || 0) >= 2.5 && Math.abs(latitude) <= 75
  }

  if (hazardName === 'Landslides') {
    return geography.isLand && ['moderate', 'high', 'severe', 'hazardous'].includes(value(row.landslide_risk)) && Math.abs(latitude) <= 75
  }

  if (hazardName === 'Industrial Emissions') {
    return geography.isLand && ['moderate', 'high', 'severe', 'hazardous', 'unhealthy', 'very unhealthy'].includes(value(row.industrial_emissions_level))
  }

  if (hazardName === 'Hazardous AQI') {
    return geography.isLand && Number(row.aqi_value || 0) >= 150
  }

  if (hazardName === 'Other Hazards') {
    return geography.isLand && !!row.other_hazards && value(row.other_hazards) !== 'none'
  }
...
export function buildHazardLayersFromPredictions(rows, predictions, hazardNames) {
  ...
  return hazardNames.reduce((layers, hazardName) => {
    ...
    layers[hazardName] = rows
      .map((row, index) => {
        if (!hasValidCoordinates(row) || !isGeographicallyAllowed(geography[index], hazardName, row)) return null
...
export function countGeographicFilters(rows, hazardNames) {
  ...
  const retainedNodes = hazardNames.reduce((count, hazardName) => {
    const filtered = rows.filter((row, index) => {
      const entry = geography[index]
      return hasValidCoordinates(row) && isGeographicallyAllowed(entry, hazardName, row)
    })
