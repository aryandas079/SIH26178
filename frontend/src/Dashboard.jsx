  ...
  return (
    <main className={`dashboard-shell min-h-screen w-full overflow-x-hidden rounded-[5px] font-instagram ${isDark ? 'bg-[#111810] text-[#edf4e5]' : 'bg-white text-[#26351b]'}`}>
      <header className={`flex min-h-20 items-center justify-between border-b px-5 py-4 md:px-10 ${isDark ? 'border-[#34452d] bg-[#182218]' : 'border-[#d7dfcd] bg-white'}`}>
        <a className={`rounded-[5px] text-base font-bold tracking-[.16em] no-underline ${isDark ? 'text-[#edf4e5]' : 'text-[#35451f]'}`} href="/">ERMS / DASHBOARD</a>
        <a className={`rounded-[5px] border px-3 py-2 text-[11px] font-bold tracking-[.1em] no-underline ${isDark ? 'border-[#a8c878] text-[#a8c878] hover:bg-[#a8c878] hover:text-[#111810]' : 'border-[#556b2f] text-[#556b2f] hover:bg-[#556b2f] hover:text-white'}`} href="/">BACK HOME</a>
      </header>
      ...
      <div className="mx-auto w-full max-w-[1800px] space-y-5 p-5 md:p-10">
        <section className={`rounded-[5px] border p-5 md:p-6 ${isDark ? 'border-[#34452d] bg-[#182218]' : 'border-[#d7dfcd] bg-white'}`}>
          <p className="mb-5 text-[11px] font-bold tracking-[.14em] text-[#718257]">01 / SAVED LOCATION</p>
          <form className="flex flex-col gap-3 md:flex-row" onSubmit={searchLocation}>
            <div className={`flex min-h-14 flex-1 rounded-[5px] border focus-within:border-[#556b2f] ${isDark ? 'border-[#50664a] bg-[#111810]' : 'border-[#b9c8ae] bg-[#fbfcfa]'}`}>
              <div className="flex min-w-0 flex-1 items-center gap-3 px-4">
                ...
                <input id="location-search" className={`min-w-0 flex-1 bg-transparent text-base outline-none ${isDark ? 'text-[#edf4e5] placeholder:text-[#718257]' : 'text-[#35451f] placeholder:text-[#8b9783]'}`} value={query} onChange={(event) => setQuery(event.target.value)} placeholder={`Search a location (current: ${location.name})`} />
              </div>
              <button className="rounded-r-[5px] bg-[#556b2f] px-6 text-[11px] font-bold tracking-wider text-white" type="submit">{searching ? 'Searching...' : 'Search location'}</button>
            </div>
            {error && <p className="mt-2 text-sm text-red-700">{error}</p>}
          </form>
          <div className={`mt-5 flex flex-wrap gap-x-6 gap-y-2 border-t pt-4 font-mono text-sm ${isDark ? 'border-[#34452d] text-[#a8c878]' : 'border-[#edf0e9] text-[#556b2f]'}`}>
        ...
        <section className={`rounded-[5px] border p-4 md:p-5 ${isDark ? 'border-[#34452d] bg-[#182218]' : 'border-[#d7dfcd] bg-white'}`}>
          {!mapOpen ? (
            <button className={`flex min-h-28 w-full flex-col items-center justify-center rounded-[5px] border border-dashed text-center transition-colors ${isDark ? 'border-[#61784e] text-[#a8c878] hover:bg-[#243220]' : 'border-[#9caf8f] text-[#556b2f] hover:bg-[#f4f6f0]'}`} type="button" onClick={() => setMapOpen(true)}>
              <span className="text-[11px] font-bold tracking-[.14em]">02 / WORLD MAP</span>
              <span className="mt-3 text-xl font-medium">Open live map</span>
              <span className="mt-1 text-sm text-[#7a8871]">View live anomaly nodes and select coordinates</span>
            </button>
          ) : (
            <>
              <div className="mb-4 flex flex-wrap items-end justify-between gap-3 px-1"><div><p className="text-[11px] font-bold tracking-[.14em] text-[#718257]">02 / WORLD MAP</p><h2 className="mt-1 text-2xl font-medium">Select a point on the map</h2></div>{selectedCoordinate && <p className="font-mono text-sm text-[#556b2f]">{selectedCoordinate[0].toFixed(4)}°, {selectedCoordinate[1].toFixed(4)}°</p>}</div>
              ...
              <button className="mt-3 rounded-[5px] border border-[#556b2f] px-3 py-2 text-[11px] font-bold tracking-wider text-[#556b2f]" type="button" onClick={() => setMapOpen(false)}>Close map</button>
            </>
          )}
        </section>

        <section className={`rounded-[5px] border p-5 ${isDark ? 'border-[#34452d] bg-[#182218]' : 'border-[#d7dfcd] bg-white'}`}>
          <p className="mb-2 text-[11px] font-bold tracking-[.14em] text-[#718257]">03 / RISK FEEDS</p>
          <p className="mb-5 text-sm text-[#7a8871]">Choose a risk layer to view the available anomaly nodes</p>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">
            {hazardTypes.map((hazard) => (
                <button className={`flex min-h-24 flex-col items-start justify-between rounded-[10px] border p-3 text-left text-sm transition-colors ${activeHazard === hazard.name ? (isDark ? 'border-[#a8c878] bg-[#304226] text-[#edf4e5]' : 'border-[#556b2f] bg-[#eef3e8] text-[#35451f]') : (isDark ? 'border-transparent bg-[#202d1e] text-[#a3b29a] hover:border-[#61784e]' : 'border-transparent bg-[#f7f9f5] text-[#64705d] hover:border-[#c5d2bd]')}`} key={hazard.name} type="button" onClick={() => { const nextNode = modelState?.layers?.[hazard.name]?.nodes?.[0] ?? null; setActiveHazard(hazard.name); setSelectedNodeId(nextNode?.id ?? null); setActiveNode(nextNode) }}>
                <span className="flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: hazard.color }} />{hazard.name}</span>
                <span><strong className="font-mono text-xl">{modelState?.layers?.[hazard.name]?.anomalyCount ?? '...'}</strong><span className="ml-1 text-[10px] uppercase tracking-wider text-[#718257]">Predicted</span></span>
              </button>
            ))}
          </div>
          <div className={`mt-5 border-t pt-4 ${isDark ? 'border-[#34452d]' : 'border-[#edf0e9]'}`}>
            <p className="text-[11px] font-bold tracking-[.1em] text-[#718257]">ACTIVE NODE</p>
            <p className="mt-2 text-base">{activeNode ? (activePlaceName || 'Resolving coordinate...') : (modelState ? 'No valid node for this hazard' : 'Loading filtered nodes...')}</p>
            <p className="mt-1 font-mono text-sm text-[#7a8871]">{displayedNode ? `${displayedNode.position[0].toFixed(4)}°, ${displayedNode.position[1].toFixed(4)}°` : 'CSV data required'}</p>
            <p className="mt-2 text-sm">{activeHazard}: <strong>{displayedNode?.value || 'No direct reading'}</strong></p>
            <p className="mt-2 text-sm text-[#c4513b]">{anomalyCount} predicted anomalies · selected score {displayedNode ? `${Math.round(displayedNode.score * 100)}%` : '0%'}</p>
            {displayedNode?.updated && <p className="mt-1 text-[11px] text-[#7a8871]">Updated: {displayedNode.updated}</p>}
            {anomalyCount > mapNodes.length && <p className="mt-1 text-[11px] text-[#7a8871]">Showing top {mapNodes.length} anomaly nodes for map performance.</p>}
          </div>
        </section>
      </div>
    </main>
